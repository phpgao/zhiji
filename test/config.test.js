...
test('默认配置加载', () => {
  ...
  assert.strictEqual(cfg.maxAnswers, 0);
  assert.strictEqual(cfg.includeFrontmatter, true);
});

test('动态修改配置并持久化', () => {
  ConfigManager.set({ vault: 'my-vault', folder: 'clips/zhihu', maxAnswers: 50 });
  const cfg = ConfigManager.get();
  assert.strictEqual(cfg.vault, 'my-vault');
  assert.strictEqual(cfg.folder, 'clips/zhihu');
  assert.strictEqual(cfg.maxAnswers, 50);
  ...
  assert.strictEqual(saved.maxAnswers, 50);
});
...
test('maxAnswers 配置值有效性处理与非负数校验', () => {
  ConfigManager.set({ maxAnswers: -10 });
  assert.strictEqual(ConfigManager.get().maxAnswers, 0);

  ConfigManager.set({ maxAnswers: '42' });
  assert.strictEqual(ConfigManager.get().maxAnswers, 42);

  ConfigManager.set({ maxAnswers: 'invalid' });
  assert.strictEqual(ConfigManager.get().maxAnswers, 0);
});

const { ScrollEngine } = require('../src/scroll.js');

test('ScrollEngine 达到 maxAnswers 上限时自动停止', async () => {
  ConfigManager.set({ maxAnswers: 10 });
  let collectionCompleted = false;
  let toastedMsg = '';
  global.App = {
    getQuestion: () => ({ answerCount: 100 }),
    answerCount: () => 10,
    onCollectionComplete: () => { collectionCompleted = true; },
  };
  global.UI = {
    toast: msg => { toastedMsg = msg; },
  };

  await ScrollEngine.start();
  assert.strictEqual(ScrollEngine.isRunning, false);
  assert.strictEqual(collectionCompleted, true);
  assert.ok(toastedMsg.includes('10 条'));
});
