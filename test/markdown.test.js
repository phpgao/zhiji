...
test('专栏文章 YAML Frontmatter 格式验证', () => {
  const q = {
    type: 'article',
    title: '深入理解现代前端工程化',
    url: 'https://zhuanlan.zhihu.com/p/718293849',
    tags: ['前端工程', 'JavaScript'],
  };
  const answers = [{
    type: 'article',
    author: 'Jimmy',
    url: 'https://zhuanlan.zhihu.com/p/718293849',
    answerUrl: 'https://zhuanlan.zhihu.com/p/718293849',
  }];
  const fm = Exporter.buildFrontmatter(q, answers);
  assert.ok(fm.includes('type: zhihu-article'));
  assert.ok(fm.includes('author: "Jimmy"'));
  assert.ok(fm.includes('source: "https://zhuanlan.zhihu.com/p/718293849"'));
  assert.ok(fm.includes('tags:\n  - zhihu\n  - 前端工程\n  - javascript'));
  assert.ok(!fm.includes('clipped_answers'), '专栏文章 frontmatter 不应包含 clipped_answers');
});

test('专栏文章 Markdown 内容完整导出 (直出正文无回答包裹)', () => {
  const q = {
    type: 'article',
    title: '深入理解现代前端工程化',
    url: 'https://zhuanlan.zhihu.com/p/718293849',
  ...
  const answers = [{
    type: 'article',
    author: 'Jimmy',
    authorUrl: 'https://www.zhihu.com/people/jimmy-dev',
    authorBio: '全栈工程师',
  ...
  assert.ok(md.includes('# 深入理解现代前端工程化'));
  assert.ok(md.includes('**作者**: [Jimmy](https://www.zhihu.com/people/jimmy-dev)'));
  assert.ok(md.includes('**来源**: [知乎专栏](https://zhuanlan.zhihu.com/p/718293849)'));
  assert.ok(md.includes('👍 520'));
  assert.ok(md.includes('💬 42条评论'));
  assert.ok(md.includes('这是文章正文第一段。'));
  assert.ok(!md.includes('的回答'), '文章模式不应包含"的回答"字样');
});

test('单回答导出包含获赞数、评论数、收藏数、喜欢数', () => {
  const q = {
    title: '现代知乎问答',
    url: 'https://www.zhihu.com/question/123',
    tags: ['测试'],
  ...
  assert.ok(md.includes('💬 10条评论'));
  assert.ok(md.includes('⭐ 3收藏'));
  assert.ok(md.includes('❤️ 1喜欢'));

  const fm = Exporter.buildFrontmatter(q, answers);
  assert.ok(fm.includes('upvotes: "58"'));
  assert.ok(fm.includes('comments_count: 10'));
  assert.ok(fm.includes('fav_count: 3'));
  assert.ok(fm.includes('likes_count: 1'));
});

test('多回答列表导出每个回答均包含获赞数、评论数、收藏数、喜欢数', () => {
  const q = {
    title: '多回答测试问题',
  ...
  const answers = [
    {
      ...
      likeCount: '4',
      content: '甲的分析内容',
      answerUrl: 'https://www.zhihu.com/question/999/answer/1'
    },
    {
      author: '答主乙',
      publishedAt: '2026-09-20',
      ...
      likeCount: '12',
      content: '乙的分析内容',
      answerUrl: 'https://www.zhihu.com/question/999/answer/2'
    }
  ];
  global.PageParser = { detectMode: () => ({ mode: 'multi' }) };
  const md = Exporter.buildMarkdown(q, answers);
  assert.ok(md.includes('👍 120'));
  assert.ok(md.includes('💬 15条评论'));
  assert.ok(md.includes('⭐ 8收藏'));
  assert.ok(md.includes('❤️ 4喜欢'));

  assert.ok(md.includes('👍 340'));
  assert.ok(md.includes('💬 45条评论'));
  assert.ok(md.includes('⭐ 20收藏'));
  assert.ok(md.includes('❤️ 12喜欢'));
});

test('专栏文章导出包含获赞数、评论数、收藏数、喜欢数', () => {
  const q = {
    type: 'article',
    title: '专栏指标测试',
    url: 'https://zhuanlan.zhihu.com/p/888',
    tags: ['专栏'],
  };
  const answers = [{
    type: 'article',
  ...
  assert.ok(md.includes('💬 66条评论'));
  assert.ok(md.includes('⭐ 99收藏'));
  assert.ok(md.includes('❤️ 55喜欢'));

  const fm = Exporter.buildFrontmatter(q, answers);
  assert.ok(fm.includes('upvotes: "888"'));
  assert.ok(fm.includes('comments_count: 66'));
  assert.ok(fm.includes('fav_count: 99'));
  assert.ok(fm.includes('likes_count: 55'));
});
