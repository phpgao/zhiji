...
// @version      1.1.0
// @description  知乎内容采集器 — 支持单回答/多回答/专栏文章导出、YAML frontmatter、Obsidian 一键存入、本地 ZIP 打包（含图片）、动态可视化配置
// @author       ZhiJi
// @icon         https://raw.githubusercontent.com/phpgao/zhiji/main/assets/logo.png
// @homepageURL  https://github.com/phpgao/zhiji
// @supportURL   https://github.com/phpgao/zhiji/issues
...
const ScrollEngine = (() => {
  ...
  function reachedTarget() {
    ...
    const count = typeof App !== 'undefined' ? App.answerCount() : 0;

    if (maxAnswers != null && count >= maxAnswers) {
      if (typeof UI !== 'undefined') UI.toast(`✅ 已达到设置的最大回答数 (${maxAnswers} 条)，自动停止`);
      stop();
...
const Exporter = (() => {
  ...
  function buildFrontmatter(q, answers) {
    const isArticle = q?.type === 'article' || (answers.length === 1 && answers[0]?.type === 'article');
    const isSingle = isArticle || (answers.length === 1 && typeof PageParser !== 'undefined' && PageParser.detectMode().mode === 'single') || answers.length === 1;
    ...
    let type = 'zhihu-question';
    if (isArticle) type = 'zhihu-article';
    else if (isSingle) type = 'zhihu-answer';
    lines.push(`type: ${type}`);
    if (isSingle && singleAnswer?.author) {
      lines.push(`author: "${singleAnswer.author.replace(/"/g, '\\"')}"`);
    }
    if (isSingle && singleAnswer) {
      if (singleAnswer.upvoteCount) lines.push(`upvotes: "${singleAnswer.upvoteCount}"`);
      if (singleAnswer.commentCount) lines.push(`comments_count: ${singleAnswer.commentCount}`);
      if (singleAnswer.favCount) lines.push(`fav_count: ${singleAnswer.favCount}`);
      if (singleAnswer.likeCount) lines.push(`likes_count: ${singleAnswer.likeCount}`);
    }
    ...
    if (!isArticle) lines.push(`clipped_answers: ${answers.length}`);
    lines.push('related: []');
    lines.push('---');
    return lines.join('\n');
  }
  ...
  function buildMarkdown(question, answers) {
    ...
    const isArticle = question?.type === 'article' || (answers.length === 1 && answers[0]?.type === 'article');
    ...
    if (isArticle) {
      const a = answers[0] || {};
      const authorShow = a.authorUrl ? `[${a.author}](${a.authorUrl})` : (a.author || '匿名用户');
      const metaParts = [];
      metaParts.push(`**作者**: ${authorShow}`);
      metaParts.push(`**来源**: [知乎专栏](${question.url || a.url})`);
      ...
      const tags = (question.tags?.length ? question.tags : a.tags) || [];
      if (cfg.includeTags && tags.length) {
        md += `> **话题**: ${tags.map(t => `#${t}`).join(' ')}\n`;
      }
      md += `> **剪藏时间**: ${formatDate()}\n\n`;
      md += '---\n\n';
    ...
    if (isSingle) {
      ...
      if (cfg.includeAnswerMeta) {
        ...
        if (a.favCount) parts.push(`⭐ ${a.favCount}收藏`);
        if (a.likeCount) parts.push(`❤️ ${a.likeCount}喜欢`);
...
const App = (() => {
  ...
  function updateExistingComments(doc = (typeof document !== 'undefined' ? document : null)) {
    ...
    if (mode !== 'single' && mode !== 'article') return;
    const cfg = typeof ConfigManager !== 'undefined' ? ConfigManager.get() : { includeComments: true };
    if (!cfg.includeComments) return;
    let updated = false;

    for (const a of answers) {
      const item = mode === 'article'
        ? (doc.querySelector('article.Post-Main') || doc.querySelector('.Post-content') || doc.body)
        : (doc.querySelector(`.AnswerItem[name="${a.answerId}"]`) || doc.querySelector('.AnswerItem'));
  ...
  function collect() {
    ...
    if (mode === 'article') {
      if (answers.length === 0 && typeof PageParser !== 'undefined') {
        const art = PageParser.parseArticle(typeof document !== 'undefined' ? document : null);
        if (art && (art.title || art.content)) {
          answers.push(art);
          knownIds.add(art.articleId || 'article');
          refresh();
        }
      }
    ...
    if (typeof PageParser !== 'undefined') {
      const cfg = typeof ConfigManager !== 'undefined' ? ConfigManager.get() : {};
      const maxAnswers = (mode === 'multi' && cfg.maxAnswers > 0) ? cfg.maxAnswers : 0;
      const limit = maxAnswers > 0 ? Math.max(0, maxAnswers - answers.length) : 0;

      if (maxAnswers > 0 && answers.length >= maxAnswers) {
        if (grabbing && !(typeof ScrollEngine !== 'undefined' && ScrollEngine.isRunning)) {
          onCollectionComplete();
          if (typeof UI !== 'undefined') {
            UI.toast(`✅ 已达到设置的最大回答数 (${maxAnswers} 条)，自动停止`);
          }
        }
        return;
      }

      const newOnes = PageParser.scanAnswers(knownIds, targetId, typeof document !== 'undefined' ? document : null, limit);
      if (newOnes.length) {
  ...
  async function startGrab() {
    ...
    if (mode === 'article') {
      ...
      if (answers.length === 0) {
        grabbing = false;
        refresh();
        if (typeof UI !== 'undefined') UI.toast('⚠️ 未能找到当前文章内容');
        return;
      }
      ...
      if (cfg.includeComments && !a.noComments && a.commentCount !== '0' && typeof PageParser !== 'undefined') {
        const item = document.querySelector('article.Post-Main') || document.querySelector('.Post-content') || document.body;
        if (item) {
      ...
      if (typeof UI !== 'undefined') {
        UI.toast(finalCommentCount > 0 ? `✅ 已剪藏当前文章与 ${finalCommentCount} 条评论` : '✅ 已剪藏当前文章');
      }
    } else if (mode === 'single') {
      if (answers.length === 0) {
        for (let i = 0; i < 10; i++) {
      ...
      refresh();
      const maxAnswers = cfg.maxAnswers > 0 ? cfg.maxAnswers : 0;
      if (maxAnswers > 0 && answers.length >= maxAnswers) {
        grabbing = false;
        refresh();
        if (typeof UI !== 'undefined') {
  ...
  function exportAs(target) {
    ...
    const q = getQuestion();
    const { mode } = currentMode();
    const exportTitle = mode === 'article'
      ? (answers[0]?.author ? `${q.title} - ${answers[0].author}` : q.title)
      : (mode === 'single' && answers[0]?.author)
