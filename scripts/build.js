...
// 3. GitHub Pages Docs
console.log('🌐 同步 GitHub Pages 静态站点资源...');
const DOCS = path.join(ROOT, 'docs');
const DOCS_ASSETS = path.join(DOCS, 'assets');
cleanAndEnsureDir(DOCS_ASSETS);
copyDir(path.join(ROOT, 'assets'), DOCS_ASSETS);
if (fs.existsSync(path.join(DOCS, 'index.html'))) {
  fs.copyFileSync(path.join(DOCS, 'index.html'), path.join(ROOT, 'index.html'));
}
console.log('  ✅ GitHub Pages 静态站点资源已同步至 docs/ 与 index.html');
