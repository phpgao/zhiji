...
try {
  ...
  const allIconsExist = [16, 32, 48, 128].every(size =>
    fs.existsSync(path.join(ICONS_DIR, `icon-${size}.png`))
  );
  if (allIconsExist) {
    console.warn('  ⚠️ 提示: 未检测到 Python PIL/Pillow 环境，使用仓库已有图标: extension/icons/');
    console.log('✨ 图标检查完成！\n');
  } else {
